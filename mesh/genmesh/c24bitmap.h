...
void TransAxis2(double &X,double&Y,double&dx,double&dy)
{
	double TmpX,TmpY;
	TmpX = X*dx  - Y*dy;
	TmpY = X*dy  + Y*dx;
...
public:
	...
	void inline DrawCircle(int x, int y, double R);
	...
	void inline DrawRotRectangle(int centerX,int centerY , double dx, double dy,
		                         double x1,double y1,double x2,double y2);
    void inline DrawRotCentRectangle(int centerX,int centerY , double dx, double dy,
								 double x1,double y1,double x2,double y2);
...
void inline C24BitMap::DrawRotCentRectangle(int centerX,int centerY, double dx, double dy,
										    double x1, double y1, double x2, double y2)
{ 
	int i;
	double PtAxisX[4];
	...
	PtAxisX[0] = x1; PtAxisY[0] = y2; 
	PtAxisX[1] = x1; PtAxisY[1] = y1; 
	PtAxisX[2] = x2; PtAxisY[2] = y1; 
	PtAxisX[3] = x2; PtAxisY[3] = y2; 
	
	Loopi(4)
	{
		//PtAxisX[i] -= centerX;
		//PtAxisY[i] -= centerY;
		TransAxis2(PtAxisX[i], PtAxisY[i], dx, dy);
	}
...
void inline C24BitMap::DrawRotRectangle(int centerX,int centerY , double dx, double dy,
		                                double x1,double y1,double x2,double y2)
{ 
	int i;
	double PtAxisX[4];
...
void inline C24BitMap::DrawCircle(int x, int y, double R)
{
   int i,j;
   for(i = x-R; i<= (x+R); i++)
	   for(j= y-R; j<= (y+R); j++)
