...
BOOL InitWater()
{
	char * waterTextFile1  =  MapCfg.GetString("WATER1","texture");


	float x = MapCfg.GetFloat("WATER1","x");
	float y = MapCfg.GetFloat("WATER1","y");
	float z = MapCfg.GetFloat("WATER1","z");
	float w = MapCfg.GetFloat("WATER1","w");
	float h = MapCfg.GetFloat("WATER1","h");
	float ax = MapCfg.GetFloat("WATER1","ax");
	float ay = MapCfg.GetFloat("WATER1","ay");

	GLtex waterText1 = g_tex_mgr.LoadTexture(waterTextFile1);
	if(waterText1 == -1)
	{
		Console.Printf("纹理文件加载错误: %s",waterTextFile1);
		TerminateApp();		
	}

	g_Water.SetTextrue(waterText1);
 	g_Water.CreateWaterPlan(x,y,z,w,h,0,0,0.9,0.5);
