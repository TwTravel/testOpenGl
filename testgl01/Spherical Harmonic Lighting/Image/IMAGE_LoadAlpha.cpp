...
bool IMAGE::LoadAlpha(const IMAGE & alphaImage)
{
	...
	if(paletted)
	{
		LOG::Instance()->OutputError("Cannot load alpha channel into a paletted image");
		return false;
	}
	...
	{
		LOG::Instance()->OutputError("Alpha image is non-paletted");
		return false;
	}
	...
	{
		LOG::Instance()->OutputError("Alpha image is not the same size as color image");
		return false;
	}
