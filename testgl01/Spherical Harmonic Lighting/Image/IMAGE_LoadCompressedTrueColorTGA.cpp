...
bool IMAGE::LoadCompressedTrueColorTGA(char * filename)
{
	...
	LOG::Instance()->OutputSuccess("Loading %s in LoadCompressedTGA", filename);
	...
	if(!tempData)
	{
		LOG::Instance()->OutputError("unable to allocate memory for temporary image data");
		fclose(file);
		return false;
	}
	...
	do
	{
		...
		{
			LOG::Instance()->OutputError("could not read RLE chunk header");
			fclose(file);
			return false;
		}
		...
		if(chunkHeader<128)		//read raw colour values
		{
			...
			for(int counter=0; counter<chunkHeader; ++counter)
			{
				...
				{
					LOG::Instance()->OutputError("unable to read image data");
					fclose(file);
					return false;
				}
				...
				{
					LOG::Instance()->OutputError("too many pixels read");
					fclose(file);
					return false;
				}
			}
