...
bool IMAGE::LoadUncompressed8BitTGA(char * filename)
{
	...
	LOG::Instance()->OutputSuccess("Loading %s in LoadUncompressed8BitTGA", filename);
	...
	{
		LOG::Instance()->OutputError("%s is not an uncompressed 8Bit TGA", filename);
		return false;
	}
	...
	if(width<=0 || height<=0 || bpp!=8)
	{
		...
		LOG::Instance()->OutputError(
			"%s's height or width is less than zero, or the TGA is not 8 bpp", filename);
		return false;
	}
	...
	{
		fclose(file);
		LOG::Instance()->OutputError("unable to allocate memory for palette");
		return false;
	}
